...
There is no ROCm-specific Dockerfile currently. If the host detects `rocm` and the runtime manifest has no `rocm` image, the daemon falls back to the `cpu` image when present.
