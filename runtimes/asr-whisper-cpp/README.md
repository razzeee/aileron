...
This runtime runs whisper.cpp through the Rust `whisper-rs` bindings and implements `speech.transcribe` and `speech.translate` for Whisper artifacts mounted under `/model`.
...
```json
{
  ...
  "runtime_id": "asr-whisper-cpp",
  "use_cases": ["speech.transcribe", "speech.translate"],
  "artifacts": [
    {
...
varlink call "unix:$XDG_RUNTIME_DIR/aileron.socket/aileron.Models.InstallUrlProfile" \
    '{"runtime_id":"asr-whisper-cpp","url":"https://huggingface.co/.../resolve/main/model.bin","sha256":"...","use_cases":["speech.transcribe","speech.translate"]}'
```
