...
fn fit_category(use_case: &str) -> Option<&'static str> {
    match use_case {
        ...
        "vision.describe" | "vision.detect" | "vision.segment" | "vision.depth" | "vision.ocr" => {
            Some("Multimodal")
        }
        _ => None,
    }
...
async fn pull_runtime_image_work(
    ...
    let result = async {
        ...
        let manifest = spawn_runtime_pull_blocking(move || {
            ...
            let inspected = remote_runtime_manifest(&pull_image_ref).ok();
            ...
            copy_image_to_oci_layout(
                inspected
                    .as_ref()
                    .map_or(pull_image_ref.as_str(), |(reference, _)| reference),
                &pull_oci_layout,
                |progress| {
